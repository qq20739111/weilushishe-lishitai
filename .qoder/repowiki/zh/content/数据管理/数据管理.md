# 数据管理

<cite>
**本文引用的文件**
- [main.py](file://src/main.py)
- [boot.py](file://src/boot.py)
- [lib/WifiConnector.py](file://src/lib/WifiConnector.py)
- [lib/SystemStatus.py](file://src/lib/SystemStatus.py)
- [lib/Logger.py](file://src/lib/Logger.py)
- [static/index.html](file://src/static/index.html)
- [static/app.js](file://src/static/app.js)
- [static/style.css](file://src/static/style.css)
- [data/config.json](file://src/data/config.json)
- [data/poems.jsonl](file://src/data/poems.jsonl)
- [data/members.jsonl](file://src/data/members.jsonl)
- [data/activities.jsonl](file://src/data/activities.jsonl)
- [data/finance.jsonl](file://src/data/finance.jsonl)
- [data/tasks.jsonl](file://src/data/tasks.jsonl)
- [data/login_logs.jsonl](file://src/data/login_logs.jsonl)
- [data/points_logs.jsonl](file://src/data/points_logs.jsonl)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向"围炉诗社·理事台"项目，围绕基于 JSONL 的流式数据存储系统，提供从数据模型、文件结构、访问模式到实现原理、CRUD 操作、性能优化、迁移与备份恢复、版本管理以及查询与缓存策略的系统化文档。重点覆盖诗歌、成员、活动、财务、事务等核心实体，以及 JsonlDB 类的设计与使用。

**更新** 项目现已重构，所有数据文件已移动到 `src/data/` 目录，提升了项目的组织性和可维护性。新增了登录日志和积分日志的数据模型，并实现了日志文件的自动清理机制。JsonlDB 类经过重大增强，包括完善的异常处理、调试输出控制和性能优化策略。

## 项目结构
项目采用"嵌入式 Web 应用 + 本地 JSONL 文件数据库"的轻量架构：
- 服务端：MicroPython 环境下的嵌入式 HTTP 服务，路由与控制器集中于主程序文件。
- 数据层：以 JSONL 文本文件作为持久化存储，分别存放各类实体数据，现位于 `src/data/` 目录。
- 前端：静态 HTML/CSS/JS，通过 API 与后端交互，同时在浏览器侧使用 IndexedDB 缓存草稿。
- 日志管理：统一的日志级别控制和输出管理，支持调试模式的动态切换。

```mermaid
graph TB
subgraph "嵌入式服务端"
MAIN["src/main.py<br/>HTTP 路由与控制器"]
BOOT["src/boot.py<br/>启动与 WiFi 连接"]
LIB["src/lib/WifiConnector.py<br/>WiFi 管理"]
SYS["src/lib/SystemStatus.py<br/>LED 状态指示"]
LOGGER["src/lib/Logger.py<br/>日志管理"]
end
subgraph "前端"
HTML["src/static/index.html<br/>页面结构"]
JS["src/static/app.js<br/>业务逻辑与缓存"]
CSS["src/static/style.css<br/>样式"]
end
subgraph "数据层(JSONL)"
POEMS["src/data/poems.jsonl"]
MEMBERS["src/data/members.jsonl"]
ACTIVITIES["src/data/activities.jsonl"]
FINANCE["src/data/finance.jsonl"]
TASKS["src/data/tasks.jsonl"]
LOGIN_LOGS["src/data/login_logs.jsonl"]
POINTS_LOGS["src/data/points_logs.jsonl"]
CONFIG["src/data/config.json"]
end
HTML --> JS
JS --> MAIN
MAIN --> POEMS
MAIN --> MEMBERS
MAIN --> ACTIVITIES
MAIN --> FINANCE
MAIN --> TASKS
MAIN --> LOGIN_LOGS
MAIN --> POINTS_LOGS
MAIN --> CONFIG
BOOT --> LIB
BOOT --> SYS
LOGGER --> MAIN
```

**图表来源**
- [main.py](file://src/main.py#L1-L1789)
- [boot.py](file://src/boot.py#L1-L153)
- [lib/WifiConnector.py](file://src/lib/WifiConnector.py#L1-L1019)
- [lib/SystemStatus.py](file://src/lib/SystemStatus.py#L1-L146)
- [lib/Logger.py](file://src/lib/Logger.py#L1-L128)
- [static/index.html](file://src/static/index.html#L1-L327)
- [static/app.js](file://src/static/app.js#L1-L2705)
- [static/style.css](file://src/static/style.css#L1-L385)
- [data/poems.jsonl](file://src/data/poems.jsonl#L1-L1)
- [data/members.jsonl](file://src/data/members.jsonl#L1-L2)
- [data/activities.jsonl](file://src/data/activities.jsonl#L1-L1)
- [data/finance.jsonl](file://src/data/finance.jsonl#L1-L1)
- [data/tasks.jsonl](file://src/data/tasks.jsonl#L1-L1)
- [data/login_logs.jsonl](file://src/data/login_logs.jsonl#L1-L34)
- [data/points_logs.jsonl](file://src/data/points_logs.jsonl#L1-L1)
- [data/config.json](file://src/data/config.json#L1-L1)

**章节来源**
- [main.py](file://src/main.py#L1-L1789)
- [boot.py](file://src/boot.py#L1-L153)
- [lib/WifiConnector.py](file://src/lib/WifiConnector.py#L1-L1019)
- [lib/SystemStatus.py](file://src/lib/SystemStatus.py#L1-L146)
- [lib/Logger.py](file://src/lib/Logger.py#L1-L128)
- [static/index.html](file://src/static/index.html#L1-L327)
- [static/app.js](file://src/static/app.js#L1-L2705)
- [static/style.css](file://src/static/style.css#L1-L385)
- [data/poems.jsonl](file://src/data/poems.jsonl#L1-L1)
- [data/members.jsonl](file://src/data/members.jsonl#L1-L2)
- [data/activities.jsonl](file://src/data/activities.jsonl#L1-L1)
- [data/finance.jsonl](file://src/data/finance.jsonl#L1-L1)
- [data/tasks.jsonl](file://src/data/tasks.jsonl#L1-L1)
- [data/login_logs.jsonl](file://src/data/login_logs.jsonl#L1-L34)
- [data/points_logs.jsonl](file://src/data/points_logs.jsonl#L1-L1)
- [data/config.json](file://src/data/config.json#L1-L1)

## 核心组件
- JsonlDB：封装 JSONL 文件的 CRUD 与分页检索，支持增量追加、按 ID 更新/删除、全文检索与分页。**更新** 新增完善的异常处理机制、调试输出控制和性能优化策略。
- 主程序路由：提供诗歌、成员、活动、财务、事务等 API，负责请求解析、参数校验与响应。
- 前端应用：负责用户交互、本地 IndexedDB 草稿缓存、与后端 API 通信。
- 启动与网络：引导 WiFi 连接、AP 备降、LED 状态指示。
- 日志管理：统一的日志级别控制和输出管理，支持调试模式的动态切换。

**章节来源**
- [main.py](file://src/main.py#L122-L336)
- [main.py](file://src/main.py#L534-L800)
- [static/app.js](file://src/static/app.js#L1-L200)
- [lib/Logger.py](file://src/lib/Logger.py#L1-L128)

## 架构总览
系统采用"嵌入式 Web 服务 + 本地 JSONL 数据库"的轻量方案，适合资源受限的嵌入式平台。前端通过 IndexedDB 在本地缓存草稿，减少频繁网络请求；后端通过 JsonlDB 提供稳定的 CRUD 与检索能力。新增的日志管理模块提供统一的日志级别控制，支持调试模式的动态切换。

```mermaid
sequenceDiagram
participant Browser as "浏览器"
participant AppJS as "前端应用(app.js)"
participant Server as "嵌入式服务(main.py)"
participant DB as "JsonlDB"
participant FS as "JSONL 文件"
participant Logger as "日志管理(Logger)"
Browser->>AppJS : 用户操作(登录/发布/编辑)
AppJS->>Server : 发送 HTTP 请求(/api/...)
Server->>Logger : 记录操作日志
Server->>DB : 解析参数并调用 CRUD/分页
DB->>FS : 追加/重写/扫描/定位
FS-->>DB : 返回记录/统计
DB-->>Server : 结果数据
Server-->>AppJS : JSON 响应
AppJS-->>Browser : 更新 UI/提示
```

**图表来源**
- [main.py](file://src/main.py#L534-L800)
- [lib/Logger.py](file://src/lib/Logger.py#L1-L128)
- [static/app.js](file://src/static/app.js#L1-L200)

## 详细组件分析

### JsonlDB 类设计与实现
- 设计目标：在嵌入式环境中提供最小化的 JSONL 数据库能力，支持高并发读场景下的稳定分页与搜索。
- 关键特性：
  - 自动目录创建与历史 .json 到 .jsonl 的迁移。
  - 追加写入 append，保证原子性（逐条写入），**新增** 完善的异常处理和错误日志记录。
  - 分页检索 fetch_page：支持倒序、搜索、限制返回数量，**更新** 优化了搜索性能和内存使用。
  - 按 ID 更新 update：全量重写文件，确保一致性，**新增** 增强的错误恢复机制。
  - 按 ID 删除 delete：全量重写文件，确保一致性，**新增** 临时文件写入确保数据完整性。
  - 获取最大 ID get_max_id：扫描文件获取最大数值 ID，**更新** 改进了 ID 解析的健壮性。
  - 全量读取 get_all：适用于小数据集（如成员、设置），**新增** 添加了异常处理。

```mermaid
classDiagram
class JsonlDB {
+string filepath
+__init__(filepath, auto_migrate)
+append(record) bool
+get_max_id() int
+fetch_page(page, limit, reverse, search_term, search_fields) (list,int)
+update(id, update_func) bool
+delete(id) bool
+get_all() list
-_ensure_dir() void
-_migrate_legacy_json() void
}
```

**图表来源**
- [main.py](file://src/main.py#L122-L336)

**章节来源**
- [main.py](file://src/main.py#L122-L336)

### 数据模型与文件结构规范
- 统一规范
  - 每条记录为一条 JSON 对象，占一行。
  - 所有实体均以 id 字段为主键，且为数值型。
  - 时间字段统一使用 ISO-8601 日期字符串（如 2026-01-23T00:00）。
  - 自定义字段统一放在 custom 字段内，键为唯一标识符（如 cf_1769341584847）。
- 实体定义
  - 诗歌 poems
    - 示例字段：author、type、content、date、id、title
    - 示例参考：[data/poems.jsonl](file://src/data/poems.jsonl#L1-L1)
  - 成员 members
    - 示例字段：id、phone、name、password、alias、role、points、joined_at、custom
    - 示例参考：[data/members.jsonl](file://src/data/members.jsonl#L1-L2)
  - 活动 activities
    - 示例字段：location、date、status、publisher、desc、title、id
    - 示例参考：[data/activities.jsonl](file://src/data/activities.jsonl#L1-L1)
  - 财务 finance
    - 示例字段：id、amount、summary、date、type、category、handler
    - 示例参考：[data/finance.jsonl](file://src/data/finance.jsonl#L1-L1)
  - 事务 tasks
    - 示例字段：reward、description、status、assignee、id、title
    - 示例参考：[data/tasks.jsonl](file://src/data/tasks.jsonl#L1-L1)
  - 登录日志 login_logs
    - 示例字段：id、member_id、member_name、phone、login_time、status
    - 示例参考：[data/login_logs.jsonl](file://src/data/login_logs.jsonl#L1-L34)
  - 积分日志 points_logs
    - 示例字段：id、member_id、member_name、change、reason、timestamp
    - 示例参考：[data/points_logs.jsonl](file://src/data/points_logs.jsonl#L1-L1)
  - 配置 config
    - 示例字段：wifi_ssid、wifi_password、ap_ssid、ap_password、points_name、password_salt、debug_mode
    - 示例参考：[data/config.json](file://src/data/config.json#L1-L1)

**章节来源**
- [data/poems.jsonl](file://src/data/poems.jsonl#L1-L1)
- [data/members.jsonl](file://src/data/members.jsonl#L1-L2)
- [data/activities.jsonl](file://src/data/activities.jsonl#L1-L1)
- [data/finance.jsonl](file://src/data/finance.jsonl#L1-L1)
- [data/tasks.jsonl](file://src/data/tasks.jsonl#L1-L1)
- [data/login_logs.jsonl](file://src/data/login_logs.jsonl#L1-L34)
- [data/points_logs.jsonl](file://src/data/points_logs.jsonl#L1-L1)
- [data/config.json](file://src/data/config.json#L1-L1)

### 数据访问模式与 API
- 诗歌 API
  - GET /api/poems：分页查询，支持 q 查询参数；默认倒序返回最新。
  - POST /api/poems：创建新作品，自动生成 id 并补全日期。
  - POST /api/poems/update：按 id 更新标题、内容、类型、日期。
  - POST /api/poems/delete：按 id 删除。
- 成员 API
  - GET /api/members：全量返回成员列表。
  - POST /api/members：创建成员，校验手机号唯一性。
  - POST /api/members/update：按 id 更新姓名、别名、电话、角色、积分、密码、自定义字段。
  - POST /api/members/delete：按 id 删除。
  - POST /api/login：按手机号与密码登录，返回脱敏用户信息。
- 活动 API
  - GET /api/activities：分页查询，支持 q 查询参数。
  - POST /api/activities：创建活动，自动生成 id。
  - POST /api/activities/update：按 id 更新标题、描述、日期、地点、状态。
  - POST /api/activities/delete：按 id 删除。
- 财务 API
  - GET /api/finance：返回最近若干条财务记录。
  - POST /api/finance：创建财务记录，自动生成 id。
- 事务 API
  - GET /api/tasks：全量返回任务列表。
  - POST /api/tasks/complete：认领并完成任务，更新任务状态与经办人，并给成员增加积分。
- 日志 API
  - GET /api/login_logs：获取登录日志（最近100条）。
- 系统 API
  - GET /api/system/info：获取系统信息（存储、内存）。
  - POST /api/migrate_passwords：迁移密码（将明文密码转换为哈希）。

**章节来源**
- [main.py](file://src/main.py#L544-L614)
- [main.py](file://src/main.py#L617-L664)
- [main.py](file://src/main.py#L667-L726)
- [main.py](file://src/main.py#L728-L791)
- [main.py](file://src/main.py#L446-L472)

### 前端数据缓存与草稿
- IndexedDB 草稿缓存
  - LocalDrafts 封装 IndexedDB，提供初始化、保存、读取、删除草稿的能力。
  - 草稿以对象形式存储，包含 id、title、type、content、author、date、isLocal 等字段。
- 前端渲染与交互
  - 登录后展示导航与功能模块；不同模块按需拉取数据。
  - 诗歌列表支持分页加载与搜索；本地草稿在首页刷新时合并展示。
  - 成员、活动、财务、事务模块提供增删改查与状态切换。
  - 任务完成时调用后端接口并更新本地积分显示。

```mermaid
flowchart TD
Start(["进入页面"]) --> CheckLogin["检查本地登录状态"]
CheckLogin --> |已登录| ShowUI["显示主界面"]
CheckLogin --> |未登录| ShowLogin["显示登录页"]
ShowUI --> FetchPoems["拉取诗歌列表(分页)"]
FetchPoems --> MergeDrafts["合并本地草稿"]
MergeDrafts --> RenderPoems["渲染诗歌卡片"]
ShowUI --> FetchMembers["拉取成员列表"]
ShowUI --> FetchActivities["拉取活动列表"]
ShowUI --> FetchFinance["拉取财务记录"]
ShowUI --> FetchTasks["拉取任务列表"]
RenderPoems --> Interact["用户交互(发布/编辑/删除)"]
Interact --> SaveDraft["保存草稿到 IndexedDB"]
Interact --> PublishPoem["发布到后端"]
PublishPoem --> Sync["同步后端与本地"]
```

**图表来源**
- [static/app.js](file://src/static/app.js#L184-L200)
- [static/app.js](file://src/static/app.js#L117-L174)

**章节来源**
- [static/app.js](file://src/static/app.js#L1-L200)
- [static/app.js](file://src/static/app.js#L117-L174)

### JsonlDB 的 CRUD 与分页算法
- 追加写入 append
  - 以追加方式写入单行 JSON，保证原子性。
  - **新增** 完善的异常处理：捕获文件写入异常并记录错误日志。
- 获取最大 ID get_max_id
  - 扫描文件，提取所有记录的 id 并取最大值，用于自增主键。
  - **更新** 改进了 ID 解析的健壮性，支持字符串 ID 的数值转换。
- 分页检索 fetch_page
  - 无搜索：先扫描文件记录行偏移，构建倒序索引，再按页读取。
  - 有搜索：全文件扫描，匹配任意字段值，再按页裁剪。
  - **更新** 优化了搜索性能：移除了调试日志输出以减少串口噪声，同时保持了异常处理。
- 按 ID 更新 update
  - 读取原文件，逐行解析并调用传入的更新函数，写入临时文件，最后替换原文件。
  - **新增** 增强的错误恢复机制：使用临时文件确保数据完整性，异常时自动清理临时文件。
- 按 ID 删除 delete
  - 与更新类似，跳过匹配的记录，其余写入临时文件并替换原文件。
  - **新增** 临时文件写入确保数据完整性，异常时自动清理临时文件。
- 全量读取 get_all
  - 适用于小数据集，如成员、设置，一次性读取全部记录。
  - **新增** 添加了异常处理，确保文件不存在时的安全返回。

```mermaid
flowchart TD
Enter(["进入 fetch_page"]) --> HasSearch{"是否有搜索关键词?"}
HasSearch --> |否| ScanOffsets["扫描文件记录偏移"]
ScanOffsets --> Reverse["按倒序排列"]
Reverse --> Slice["按页切片"]
Slice --> ReadLines["定位并读取行"]
ReadLines --> ReturnFast["返回结果与总数"]
HasSearch --> |是| FullScan["全文件扫描匹配"]
FullScan --> MaybeReverse["按倒序(可选)"]
MaybeReverse --> Paginate["按页裁剪"]
Paginate --> ReturnSlow["返回结果与总数"]
```

**图表来源**
- [main.py](file://src/main.py#L184-L261)

**章节来源**
- [main.py](file://src/main.py#L155-L164)
- [main.py](file://src/main.py#L165-L183)
- [main.py](file://src/main.py#L184-L261)
- [main.py](file://src/main.py#L262-L336)

### 日志管理与数据备份清理
- 日志管理
  - Logger 类提供统一的日志级别控制，支持 DEBUG/INFO/WARN/ERROR 四个级别。
  - 通过 config.json 中的 debug_mode 字段控制日志输出级别。
  - 支持动态设置调试模式，便于运行时调试。
- 登录日志清理
  - record_login_log 函数自动记录登录日志并维护最多100条记录。
  - 当日志数量超过100条时，自动清理最旧的记录，保留最新的100条。
  - 使用临时文件写入方式，确保数据一致性。
  - **新增** 增强了清理过程的异常处理，确保即使清理失败也不会影响系统稳定性。

```mermaid
flowchart TD
Start(["记录登录日志"]) --> CreateLog["创建日志记录"]
CreateLog --> AppendFile["追加到 login_logs.jsonl"]
AppendFile --> CheckCount{"检查日志数量"}
CheckCount --> |<= 100| End["完成"]
CheckCount --> |> 100| GetAll["读取所有日志"]
GetAll --> KeepLatest["保留最新100条"]
KeepLatest --> WriteTmp["写入临时文件"]
WriteTmp --> ReplaceFile["替换原文件"]
ReplaceFile --> End
```

**图表来源**
- [main.py](file://src/main.py#L446-L472)
- [lib/Logger.py](file://src/lib/Logger.py#L1-L128)

**章节来源**
- [main.py](file://src/main.py#L446-L472)
- [lib/Logger.py](file://src/lib/Logger.py#L1-L128)

### 数据迁移、备份与版本管理
- 迁移策略
  - JsonlDB 支持自动检测历史 .json 文件并迁移到 .jsonl，避免数据丢失。
  - 迁移过程读取旧数据，逐条写入新格式，完成后可选择删除旧文件。
  - **新增** 改进了迁移过程的异常处理，确保迁移失败时不会破坏现有数据。
- 备份与恢复
  - 建议定期复制 `src/data/` 目录中的 JSONL 文件与 config.json。
  - 恢复时将对应文件放回原路径，重启服务即可。
- 版本管理
  - 字段演进遵循向后兼容：新增字段不影响既有记录读取。
  - 自定义字段通过 config.json 的 custom_member_fields 管理，便于扩展。

**章节来源**
- [main.py](file://src/main.py#L137-L154)
- [data/config.json](file://src/data/config.json#L1-L1)

### 查询、索引与缓存
- 查询
  - 分页查询：支持 page、limit、q 参数；默认倒序返回最新。
  - 搜索：在无索引情况下进行全字段字符串匹配，注意性能影响。
  - **更新** 优化了搜索性能：移除了调试日志输出以减少串口噪声。
- 索引
  - 当前未实现专用索引；可通过自定义字段键名与固定字段组合提升搜索效率。
- 缓存
  - 浏览器端：IndexedDB 本地草稿缓存，避免重复网络请求。
  - 服务端：未实现进程内缓存；建议在高频读场景下引入只读缓存或只在特定 API 使用。

**章节来源**
- [main.py](file://src/main.py#L544-L567)
- [static/app.js](file://src/static/app.js#L184-L200)
- [static/app.js](file://src/static/app.js#L117-L174)

## 依赖分析
- 启动与网络
  - boot.py 通过 WifiConnector 连接 WiFi，失败则启动 AP；SystemStatus 控制 LED 状态。
- 服务端
  - main.py 注入 Microdot，注册路由并实例化 JsonlDB；处理登录、CRUD、系统信息等。
  - Logger 提供统一的日志管理功能。
- 前端
  - static/index.html 提供页面骨架；static/app.js 负责业务逻辑与 IndexedDB；static/style.css 提供样式。

```mermaid
graph LR
BOOT["src/boot.py"] --> WIFI["src/lib/WifiConnector.py"]
BOOT --> SYS["src/lib/SystemStatus.py"]
MAIN["src/main.py"] --> JSONL["JsonlDB"]
MAIN --> ROUTES["路由与控制器"]
MAIN --> LOGGER["Logger"]
APPJS["src/static/app.js"] --> MAIN
INDEX["src/static/index.html"] --> APPJS
STYLE["src/static/style.css"] --> INDEX
LOGGER --> MAIN
```

**图表来源**
- [boot.py](file://src/boot.py#L1-L153)
- [lib/WifiConnector.py](file://src/lib/WifiConnector.py#L1-L1019)
- [lib/SystemStatus.py](file://src/lib/SystemStatus.py#L1-L146)
- [main.py](file://src/main.py#L1-L1789)
- [lib/Logger.py](file://src/lib/Logger.py#L1-L128)
- [static/index.html](file://src/static/index.html#L1-L327)
- [static/app.js](file://src/static/app.js#L1-L2705)
- [static/style.css](file://src/static/style.css#L1-L385)

**章节来源**
- [boot.py](file://src/boot.py#L1-L153)
- [lib/WifiConnector.py](file://src/lib/WifiConnector.py#L1-L1019)
- [lib/SystemStatus.py](file://src/lib/SystemStatus.py#L1-L146)
- [main.py](file://src/main.py#L1-L1789)
- [lib/Logger.py](file://src/lib/Logger.py#L1-L128)
- [static/index.html](file://src/static/index.html#L1-L327)
- [static/app.js](file://src/static/app.js#L1-L2705)
- [static/style.css](file://src/static/style.css#L1-L385)

## 性能考虑
- I/O 模式
  - JSONL 逐行存储，append 为顺序写，适合高写入吞吐。
  - 分页检索在无搜索时通过偏移定位，避免全量读取；有搜索时为全表扫描，建议控制搜索范围与字段。
  - **更新** 优化了搜索性能：移除了调试日志输出以减少串口噪声，同时保持了异常处理。
- 内存与 CPU
  - 更新/删除采用全文件重写，注意在大数据量时的 I/O 峰值；可在业务侧降低频繁更新频率。
  - 前端 IndexedDB 仅缓存草稿，避免占用过多内存。
- 网络与并发
  - 嵌入式平台并发有限，建议合理设置分页大小与搜索条件，避免长时间阻塞。
- 存储空间
  - 定期清理历史数据与冗余记录，监控文件大小与剩余空间。
  - 登录日志自动清理机制确保不会产生过多的历史数据。
  - **新增** 增强了清理过程的异常处理，确保即使清理失败也不会影响系统稳定性。

## 故障排查指南
- 登录失败
  - 检查 `src/data/members.jsonl` 中是否存在对应手机号与密码；确认请求体字段正确。
  - 参考：[main.py](file://src/main.py#L586-L614)
- 无法连接网络
  - 检查 `src/data/config.json` 的 SSID/密码；查看 boot.py 的连接日志与 AP 启动流程。
  - 参考：[boot.py](file://src/boot.py#L28-L105)，[lib/WifiConnector.py](file://src/lib/WifiConnector.py#L1-L200)
- 任务完成积分未更新
  - 检查 /api/tasks/complete 的调用参数与返回；确认成员列表中存在对应用户。
  - 参考：[main.py](file://src/main.py#L728-L791)，[static/app.js](file://src/static/app.js#L1-L200)
- 前端草稿无法保存
  - 检查浏览器 IndexedDB 支持与权限；查看 LocalDrafts 的初始化与 put/delete 调用。
  - 参考：[static/app.js](file://src/static/app.js#L117-L174)
- 日志文件过大
  - 检查登录日志清理机制是否正常工作；确认 record_login_log 函数的调用。
  - 参考：[main.py](file://src/main.py#L446-L472)
- **新增** JsonlDB 操作失败
  - 检查日志输出，确认是否为调试模式；查看错误日志中的具体异常信息。
  - 确认文件权限和磁盘空间；检查临时文件是否被意外清理。
  - 参考：[main.py](file://src/main.py#L155-L163)，[main.py](file://src/main.py#L293-L295)

**章节来源**
- [main.py](file://src/main.py#L586-L614)
- [boot.py](file://src/boot.py#L28-L105)
- [lib/WifiConnector.py](file://src/lib/WifiConnector.py#L1-L200)
- [main.py](file://src/main.py#L728-L791)
- [static/app.js](file://src/static/app.js#L117-L174)
- [main.py](file://src/main.py#L446-L472)
- [main.py](file://src/main.py#L155-L163)
- [main.py](file://src/main.py#L293-L295)

## 结论
本项目以 JSONL 为核心的数据存储方案，结合嵌入式 Web 服务与前端 IndexedDB 草稿缓存，实现了轻量、可靠、易部署的诗社数据管理平台。JsonlDB 经过重大增强，提供了完善的异常处理机制、调试输出控制和性能优化策略，显著提升了系统的稳定性和可靠性。项目现已重构，数据文件位于 `src/data/` 目录，提升了项目的组织性和可维护性。新增的日志管理模块提供了统一的日志级别控制，支持调试模式的动态切换。登录日志的自动清理机制确保了系统的长期稳定运行。建议在后续迭代中引入更高效的索引与缓存策略，以进一步提升性能与用户体验。

## 附录
- 快速参考
  - 诗歌：/api/poems
  - 成员：/api/members
  - 活动：/api/activities
  - 财务：/api/finance
  - 事务：/api/tasks/complete
  - 登录：/api/login
  - 登录日志：/api/login_logs
  - 系统信息：/api/system/info
  - 密码迁移：/api/migrate_passwords