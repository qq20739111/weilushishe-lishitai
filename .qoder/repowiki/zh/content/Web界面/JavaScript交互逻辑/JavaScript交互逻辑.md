...
**更新** 本版本重点反映了新增的IndexedDB本地草稿功能、完整的用户认证系统和SPA架构实现。
...
**图表来源**
- [index.html](file://src/static/index.html#L1-L321)
- [app.js](file://src/static/app.js#L46-L98)
- [main.py](file://src/main.py#L284-L291)
- [boot.py](file://src/boot.py#L1-L122)

**章节来源**
- [index.html](file://src/static/index.html#L1-L321)
- [app.js](file://src/static/app.js#L1-L1603)
...
- **用户认证系统**：基于localStorage存储用户信息，登录成功后隐藏登录区并显示主应用，支持密码哈希验证。
...
**章节来源**
- [app.js](file://src/static/app.js#L100-L145)
- [app.js](file://src/static/app.js#L46-L98)
- [app.js](file://src/static/app.js#L150-L188)
- [app.js](file://src/static/app.js#L1264-L1371)

## 架构总览
前端通过fetch调用后端API，后端以Microdot提供路由，数据持久化于JSON Lines文件。设备侧启动脚本负责网络初始化与应用启动。新增的IndexedDB本地草稿系统为前端提供了离线草稿存储能力。
...
participant A as "后端API(main.py)"
participant S as "设置文件(settings.json)"
U->>V : 打开页面
V->>J : 加载脚本
J->>J : 初始化与检查登录
J->>D : 初始化IndexedDB
...
**图表来源**
- [app.js](file://src/static/app.js#L116-L139)
- [main.py](file://src/main.py#L551-L574)
...
**图表来源**
- [app.js](file://src/static/app.js#L46-L98)
- [app.js](file://src/static/app.js#L385-L409)
- [app.js](file://src/static/app.js#L411-L444)
...
**章节来源**
- [app.js](file://src/static/app.js#L147-L188)
...
**图表来源**
- [app.js](file://src/static/app.js#L206-L254)
- [app.js](file://src/static/app.js#L233-L249)

**章节来源**
- [app.js](file://src/static/app.js#L206-L254)
- [app.js](file://src/static/app.js#L265-L329)
...
**图表来源**
- [app.js](file://src/static/app.js#L190-L194)
...
### 实时数据更新与轮询机制
- **当前实现**：未使用WebSocket或长轮询，主要通过页面切换与手动刷新触发数据拉取。
- **可选优化**：对高频变更场景（如财务流水、活动状态）可引入WebSocket或短周期轮询，结合缓存与增量更新策略减少重复请求。
- **系统监控**：通过/system/info接口定期获取设备状态信息。

**章节来源**
- [app.js](file://src/static/app.js#L1106-L1190)
- [main.py](file://src/main.py#L644-L656)
...
**章节来源**
- [app.js](file://src/static/app.js#L182-L188)
- [app.js](file://src/static/app.js#L297-L299)

## 依赖关系分析
- **前端依赖**：index.html引入app.js与style.css；app.js依赖fetch API与localStorage；使用IndexedDB进行本地草稿存储。
- **后端依赖**：main.py依赖Microdot框架与文件系统；数据层封装JsonlDB类，提供分页、搜索、更新、删除等能力。
...
**章节来源**
- [app.js](file://src/static/app.js#L1304-L1310)
- [app.js](file://src/static/app.js#L1384-L1391)
- [app.js](file://src/static/app.js#L206-L254)
...
**章节来源**
- [main.py](file://src/main.py#L551-L574)
- [app.js](file://src/static/app.js#L1304-L1357)
- [app.js](file://src/static/app.js#L239-L242)

## 结论
本项目前端以轻量脚本实现完整的业务交互，结合后端REST API与JSON Lines数据存储，形成简洁高效的本地化管理系统。通过统一的页面切换、完善的表单校验与权限控制、并发搜索与缓存策略，实现了良好的用户体验。新增的IndexedDB本地草稿功能和完整的用户认证系统进一步提升了应用的实用性和安全性。未来可在高频变更场景引入WebSocket或轮询机制，并进一步优化内存与渲染性能。
